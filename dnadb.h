...
//Professor Johnson 
//1 May 2022 
//Description: This is the header file provided with added private helper functions.
...
#include "math.h"
using namespace std;
class Grader;   // forward declaration, will be used for grading
...
private:
    ...
    ******************************************/
    bool lookCurrent(string sequence, int location, int t_size);
    bool lookOld(string sequence, int location, int t_size);
    int quadProbeHash(string key);
    int quadProbeHashOld(string key);
    DNA* newTable();
    void divisionInsert();
    int quadProbeRemove(string key, int capacity);
    void quarterInsert();
};
