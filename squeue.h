...
private:
    ...
    void printRecurser(Node* tempNode) const;
    void clearRecurser(Node* aNode);
    int priorityFn1(const Post& post);//for MAXHEAP
    int priorityFn2(const Post& post);//for MINHEAP
    Node* mergeHelper(Node* p1, Node* p2);
    void queueDestructor(Node* aNode);
    Node* copyRecurser(Node* oldNode);
